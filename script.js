/// CONFIGURATION
var partocleCount = 100;
var particleSpeed = 10; // Number of millisecondes 
...
function createCursor(position) {
  ...
  if (!position) {
    var pos = {
      x: (SCREEN_WIDTH / 2) * 0.5 + (Math.random() * w),
      y: (SCREEN_HEIGHT / 2) * 0.5 + (Math.random() * h)
    };
...
function loop() {
  ...
  for (i = 0, ilen = particles.length; i < ilen; i++) {
    ...
    context.beginPath();
    context.fillStyle = "hsl(203, 100%, 39%)";
...
function Particle() {
  ...
  this.angle = 0;
// Enable math random to have different speed to every particle
//  this.speed = 0.01 + Math.random() * particleSpeed;
  this.speed =  particleSpeed / 1000;
